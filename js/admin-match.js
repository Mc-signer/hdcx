	...
	$.post("php/teamController.php",{"key":"getTeams"},function(data){
       ...
       if (data.length == 0) $(".main-admin-match-team-content .none").css("display", "block"); 
		...
		for (var i = 0;i <= data.teams.length - 1;i++) { 
			...
			$(".main-admin-match-team-content").append("<div class='col-md-12 main-right-match-content-detail' id='" + data.teams[i].id + "' style='display:block;'><div class='col-md-9' onclick='viewTeam(this);'><div class='col-md-12 detail-head'><div class='col-md-12 detail-head-left'>" + data.teams[i].team_name + "</div></div><div class='col-md-12 detail-body-now'>" + data.teams[i].pro_intro + "</div></div><div class='col-md-3'><h3>" + state[statusNumber] + "</h3></div>    </div>");
...
function viewTeam(element){
     window.location.href="admin-output.html";
     sendTeamId(element);
